...
def insert_new_file():
	...
	### Hash function SHA1 (~160bit-20byte) ### 
	m = hashlib.sha1()
...
def query_file():
	...
	if(client_ip is None or client_port is None):
		client_ip = request.environ['REMOTE_ADDR'] #  "REMOTE_ADDR": "192.168.0.131"
		client_port = request.environ['REMOTE_PORT'] #  "REMOTE_PORT": 53690
		direct_reply = False
	if(unhashed_key == '*'):
