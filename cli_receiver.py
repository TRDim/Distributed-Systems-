...
# hashlib: Create Hash funstions 
# urllib: Library for opening URLs
# signal: for custom handlers, called by the target environment when the corresponding signal occurs. Suspend execution the system until the the signal handler receives a Call(+)
# sys: commands to interact with the interpreter 
# bisect:  This module provides support for maintaining a list in sorted order without having to sort the list after each insertion
# json: format for transmitting data between a web application and a server 

from flask import Flask, make_response, jsonify, Response
# make_response: instead of a return- to attach headers 
# jsonify: creates response with the JSON 
# Response: to get the server's response to the HTTP request

from flask import request
from gevent import monkey
# gevent: high-level API
# monkey: make cooperative the standard library 
...
def run_requests_from_file_cli():
    ...
    for line in Lines:
        ...
        #Pass query to the corresponding node
        if type == 'query':
            response = requests.get(url = "http://"+str(ip)+":"+str(port)+"/query", params = {'key':key, 'client_ip':MY_IP, 'client_port':sys.argv[1]+"/ \q2ubatch_query_result_cli"})
